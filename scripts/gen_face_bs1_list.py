...
def get_nonfaces():
    nonfaces = {}
    lines = []
    lines = os.listdir(BOOTSTRAP1_ROOT)
    # lines = lines[:10]
    nonfaces = list_split(lines)
    dict_insert_root(nonfaces, BOOTSTRAP1_ROOT)
    return nonfaces
...
def save_list(faces, nonfaces):
    for k in FACE_BS1_TRAIN_LISTS.keys():
        with open(FACE_BS1_TRAIN_LISTS[k], 'w') as f:
...
def main():
    ...
    dict_insert_root(single_split, BOOTSTRAP1_ROOT)
